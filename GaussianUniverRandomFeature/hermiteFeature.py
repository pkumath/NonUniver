...
def vectorized_hermite_features(X, k):
    """
    ...
    # Sort degree combinations according to the desired order
    # First by number of non-zero entries, then by position of the first non-zero entry, then lex order
    d_max = d  # Maximum possible index
    degree_combinations.sort(
        key=lambda t: (
            sum(1 for ti in t if ti != 0),  # Number of non-zero entries
            next((i for i, ti in enumerate(t) if ti != 0), d_max),  # Index of first non-zero entry
            t  # Lexicographical order
        )
    )
    # print('Degree combinations in vectorized_hermite_features:', degree_combinations)
    
    # Prepare a list to collect unique feature vectors
    Z = []
    for t in degree_combinations:
        # For each degree combination, compute the product of univariate Hermite polynomials
        He_prod = np.prod([He[:, j, t_j] for j, t_j in enumerate(t)], axis=0)  # Shape: (n_samples,)
        Z.append(He_prod)
    
    # Stack all feature vectors horizontally to form the feature matrix Z
    Z = np.column_stack(Z)  # Shape: (n_samples, p)
    return Z
