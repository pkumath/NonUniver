...
def empirical_risk_minimization_leave_one_out(X, y, k, lambda_reg, loss_function, loss_function_grad, return_grad=False, leave_out_index=0):
    """
    Perform empirical risk minimization to find optimal T.
    """
    # Delete the leave_out_index sample of X and y
...
def empirical_risk_minimization_constrained_direction(
    X, y, k, lambda_reg, loss_function, loss_function_grad, return_grad=False, constrained_direction=None
):
    """
    Perform empirical risk minimization to find optimal T under the constraint that T symmetrically contracts with constrained_direction to zero.
    """
    n_samples, d = X.shape
    # Determine tensor shape
    tensor_shape = tuple([d] * k)
    ...
    # Define constraints if constrained_direction is given
    constraints = []
    if constrained_direction is not None:
        u = np.asarray(constrained_direction)
        # Define the constraint function
        def cons_f(T_vec):
            T = unflatten_tensor(T_vec, tensor_shape)
            # Contract T with u over the first index
            contraction = np.tensordot(u, T, axes=(0, 0))  # Shape: (d, d, ..., d), order k-1
            contraction_flat = contraction.flatten()
            return contraction_flat
        constraints = [{'type': 'eq', 'fun': cons_f}]
    ...
    T_opt = unflatten_tensor(result.x, tensor_shape)
    if return_grad:
        grad = (1 / n_samples) * (He_flat.T @ loss_function(y, He_flat @ result.x))
        return T_opt, unflatten_tensor(grad, tensor_shape)
    return T_opt
...
if __name__ == "__main__":
    ...
    T_hat = empirical_risk_minimization_leave_one_out(X, y, k, lambda_reg, loss_function, loss_function_grad)
