...
if __name__ == "__main__":
    ...
    for d in range(10, 70):
        ...
        ba_full, _ = empirical_risk_minimization(
            X_full, y_full, W, lambda_reg, loss_function, loss_function_grad,
            scaling_factor=scaling_factor, exclude_index=None
        )
        ...
        ba_tilde_k, ba_excl_k = approximate_empirical_risk_minimization(
            X_full, y_full, W, lambda_reg, loss_function, loss_function_grad,
            loss_function_hess, scaling_factor=scaling_factor, exclude_index=k_index
        )

        # Compute distances between minimizers
        dist_full_vs_excl = np.linalg.norm(ba_full - ba_excl_k, 2)
        dist_full_vs_approx = np.linalg.norm(ba_full - ba_tilde_k, 2)
        dist_excl_vs_approx = np.linalg.norm(ba_excl_k - ba_tilde_k, 2)
    ...
    name = ''
    fig, ax = plt.subplots()
    # ax.plot(list(distance_full_vs_excl.keys()), list(distance_full_vs_excl.values()), label='Distance: Full vs Excl k')
    # name += 'Full_vs_Excl_k'
    ax.plot(list(distance_full_vs_approx.keys()), list(distance_full_vs_approx.values()), label='Distance: Full vs Approx')
    name += '_Full_vs_Approx'
    # ax.plot(list(distance_excl_vs_approx.keys()), list(distance_excl_vs_approx.values()), label='Distance: Excl k vs Approx')
    ...
    exponent = 1.5
    ax.plot(list(distance_full_vs_excl.keys()), 0.02* np.log(list(distance_full_vs_excl.keys())) / np.array(list(distance_full_vs_excl.keys())) ** exponent, '--', label=f'1/d^{exponent}')
    ...
    fig.savefig(f"figures/Dec3/Distance_vs_Dimension_{loss_function.__name__}_lambda_{lambda_reg}_alpha_{alpha}_signal_{signal}_scaling_{scaling_factor}_{name}.png")